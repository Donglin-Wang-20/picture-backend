 ...
 * @createDate 2025-05-28 22:02:13
 */
@Service
@Slf4j
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture>
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User loginUser) {
        ...
        // 如果是更新图片，需要校验图片是否存在 并且 只有管理员和自己可以修改图片
        if (pictureId != null) {
     ...
     */
    /**
     * 分页查询 Picture → List<Picture>
     * ↓
     * 转成 List<PictureVO>
     * ↓
     * 收集 userId → Set<Long>
     * ↓
     * 查所有 User → Map<Long, List<User>>
     * ↓
     * 遍历 PictureVO → 填充 UserVO
     * ↓
     * setRecords → 返回 Page<PictureVO>
     */
    @Override
    public Page<PictureVO> getPictureVOPage(Page<Picture> picturePage, HttpServletRequest request) {
        // 拿到分页里的图片数据
        List<Picture> pictureList = picturePage.getRecords();
        // 新建一个空的 PictureVO 分页对象，准备装数据
        ...
        /*
            ...
            for (Picture picture : pictureList) {
                Long userId = picture.getUserId();
                userIdSet2.add(userId);
            }
        */
        // 一次性查所有用户信息
        // 一次性把这些用户都查出来，按 id 分组放 map，避免循环里一条条查（N+1 查询）
        ...
        // 遍历所有 PictureVO
        // 看 map 里有没有这个 userId
        // 有就取出来第一个（因为 map 值是 List<User>）
        // 再用 userService.getUserVo(user) 转成前端需要的 UserVO 格式，塞进 PictureVO
    ...
    public void doPictureReview(PictureReviewRequest pictureReviewRequest, User loginUser) {
        ...
        // id 审核状态不能为空 不能把审核通过和审核未通过的图片改为待审核
        ...
        // 将传递过来的参数数据赋值到新创建的updatePicture中，因为操作数据库需要用的Picture对象
    ...
    public Integer uploadPictureByBatch(PictureUploadByBatchRequest pictureUploadByBatchRequest, User loginUser) {
        ...
        for (Element imgElement : imgElementList) {
            ...
            try {
                ...
                log.info("图片名称，name = {}", pictureVO.getName());
                uploadCount++;
            } catch (Exception e) {
                log.error("图片上传失败", e);
                continue;
